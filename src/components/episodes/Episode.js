...
const Episode = ({ auth }) => {
  ...
  return (
    <div className="container card">
      {episode ? (
        <div className="m-2">
          <h2 className="text-center">{episode.original_title}</h2>
          <div className="row m-2">
            <Image
              src={
                process.env.REACT_APP_API_LINK +
                "/pictures/episodes?key=" +
                process.env.REACT_APP_API_KEY +
                "&id=" +
                episodeid +
            ...
            <div className="col-sm-12 col-md-6 col-lg-9 col-xl-9">
              ...
              <p>
                <Rating
                  ...
                  defaultValue={episode.note.mean}
                  precision={0.1}
                  readOnly
                />
              </p>
              <p>Synopsis: {episode.description}</p>
              <Link to={`/show/${episode.show.id}`}>
                <Button>Go Back to the Show Page</Button>
              </Link>
            </div>
          </div>
          <h3 className="pt-3">Actors</h3>
          <ItemsCarousel
